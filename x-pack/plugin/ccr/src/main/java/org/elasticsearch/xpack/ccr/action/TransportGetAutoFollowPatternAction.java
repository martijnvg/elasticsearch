...
public class TransportGetAutoFollowPatternAction
    ...
    protected void masterOperation(GetAutoFollowPatternAction.Request request,
        ...
        Map<String, AutoFollowPattern> autoFollowPatterns = getAutoFollowPattern(state.metaData(), request.getRemoteCluster());
    ...
    static Map<String, AutoFollowPattern> getAutoFollowPattern(MetaData metaData, String remoteCluster) {
        AutoFollowMetadata autoFollowMetadata = metaData.custom(AutoFollowMetadata.TYPE);
        if (autoFollowMetadata == null) {
            throw new ResourceNotFoundException("no auto-follow patterns for remote cluster [{}] found", remoteCluster);
        }
        ...
        AutoFollowPattern autoFollowPattern = autoFollowMetadata.getPatterns().get(remoteCluster);
        if (autoFollowPattern == null) {
            throw new ResourceNotFoundException("no auto-follow patterns for remote cluster [{}] found", remoteCluster);
        }
        return Collections.singletonMap(remoteCluster, autoFollowPattern);
    }
}
