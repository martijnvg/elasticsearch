...
public class MvMinBytesRefsFromBinaryBlockLoaderTests extends ESTestCase {
    ...
    public MvMinBytesRefsFromBinaryBlockLoaderTests(boolean blockAtATime, boolean multiValues, boolean missingValues) {
        this.blockAtATime = blockAtATime;
        this.multiValues = multiValues;
        this.missingValues = missingValues;
    }

    public void testMvMinBinaryDocValues() throws IOException {
        try (Directory dir = newDirectory(); RandomIndexWriter iw = new RandomIndexWriter(random(), dir)) {
            ...
            for (int i = 0; i < docCount; i++) {
                ...
                field.add(new BytesRef(valueFor(i, cardinality, "v")));
                int count = 1;
                if (multiValues && i % cardinality == 0) {
                    field.add(new BytesRef(valueFor(i + 1, cardinality, "a")));
                    count = 2;
                }
                NumericDocValuesField countField = NumericDocValuesField.indexedField(field.countFieldName(), count);
            ...
            try (DirectoryReader dr = iw.getReader()) {
                ...
                try (
                    ...
                    TestBlock minStrings = read(mvMinLoader, mvMinReader, docs);
                ) {
    ...
    private Matcher<Object> readerMatcher() {
        if (multiValues) {
            return hasToString("MvMinBytesRefsFromBinary.SeparateCounts");
        }
    ...
    private void checkBlocks(TestBlock strings, TestBlock mvMin) {
        for (int i = 0; i < strings.size(); i++) {
            ...
            BytesRef bytes = (BytesRef) (str instanceof List<?> l ? minValue(l) : str);
            assertThat(mvMin.get(i), equalTo(bytes));
        }
    }

    private BytesRef minValue(List<?> values) {
        BytesRef min = null;
        for (Object value : values) {
            BytesRef bytes = (BytesRef) value;
            if (min == null || bytes.compareTo(min) < 0) {
                min = bytes;
            }
        }
        return min;
    ...
    private static String valueFor(int index, int cardinality, String prefix) {
        return String.format(Locale.ROOT, "%s%05d", prefix, index % cardinality);
    }
}
