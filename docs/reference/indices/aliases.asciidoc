...
Fields in alias filters pointing to a index created on version `1.4.0` or later
need to exist in the mappings.
...
First if not already exists the field the filter is referring to:
...
If not already exists create the index with field `user_id`:
...
Add the alias for a specific user:
