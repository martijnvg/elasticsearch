...
Except for queries and filter defined in filtered aliases or percolator queries, referring the field names not existing
in the mappings is allowed. Whether unmapped fields are allowed in queries or filters can be controlled via the
following the `index.query.parse.allow_unmapped_fields` index setting.
