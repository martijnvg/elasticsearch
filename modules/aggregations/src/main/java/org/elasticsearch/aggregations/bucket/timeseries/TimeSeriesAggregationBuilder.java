...
public class TimeSeriesAggregationBuilder extends AbstractAggregationBuilder<TimeSeriesAggregationBuilder> {
    ...
    private BucketMetricsPipelineAggregationBuilder<?> pipeline;
    ...
    public void setPipeline(PipelineAggregationBuilder pipeline) {
        if (pipeline instanceof BucketMetricsPipelineAggregationBuilder<?> bmpa) {
            this.pipeline = bmpa;
        } else {
            throw new IllegalArgumentException("only bucket metrics pipeline aggregations are supported");
        }
    }
