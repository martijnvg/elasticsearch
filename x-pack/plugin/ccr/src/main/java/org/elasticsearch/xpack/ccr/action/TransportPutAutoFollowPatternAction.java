...
public class TransportPutAutoFollowPatternAction extends
    ...
    static ClusterState innerPut(PutAutoFollowPatternAction.Request request,
        ...
        AutoFollowPattern previousPattern = patterns.get(request.getRemoteCluster());
        final List<String> followedIndexUUIDs;
        if (followedLeaderIndices.containsKey(request.getRemoteCluster())) {
            followedIndexUUIDs = new ArrayList<>(followedLeaderIndices.get(request.getRemoteCluster()));
        } else {
            followedIndexUUIDs = new ArrayList<>();
        }
        followedLeaderIndices.put(request.getRemoteCluster(), followedIndexUUIDs);
        ...
        if (filteredHeaders != null) {
            headers.put(request.getRemoteCluster(), filteredHeaders);
        }
        ...
        patterns.put(request.getRemoteCluster(), autoFollowPattern);
