    ...
    public void testBinaryDocValues() throws IOException {
        try (Directory dir = newDirectory(); RandomIndexWriter iw = new RandomIndexWriter(random(), dir)) {
            ...
            for (int i = 0; i < docCount; i++) {
                ...
                // Use different strings to test min comparison
                String value1 = "b".repeat(i % cardinality);
                field.add(new BytesRef(value1));
                NumericDocValuesField countField;
                if (multiValues && i % cardinality == 0) {
                    // Add a second value that is sometimes smaller, sometimes larger
                    String value2 = (i % 2 == 0) ? "a".repeat((i % cardinality) + 1) : "c".repeat((i % cardinality) + 1);
                    field.add(new BytesRef(value2));
            ...
            try (DirectoryReader dr = iw.getReader()) {
                ...
                if (multiValues) {
                    assertThat(mvMinReader, hasToString("MvMinBytesRefsFromBinary"));
                } else {
                    assertThat(mvMinReader, hasToString("MvMinBytesRefsFromBinary.SingleValued"));
                }
