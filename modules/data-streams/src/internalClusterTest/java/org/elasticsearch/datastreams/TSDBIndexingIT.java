...
public class TSDBIndexingIT extends ESSingleNodeTestCase {
    ...
    public void testTrimIdSeqNoAndRecoverySource() throws Exception {
        String dataStreamName = "k8s";
        var putTemplateRequest = new TransportPutComposableIndexTemplateAction.Request("id");
        putTemplateRequest.indexTemplate(
            ComposableIndexTemplate.builder()
                ...
                .entry(
                    ...
                    matchesMap().extraOk()
                        .entry(
                            "fields",
                            matchesMap().extraOk()
                                .entry("_id", matchesMap().extraOk().entry("total_in_bytes", greaterThanOrEqualTo(1)))
                                .extraOk()
                                .entry("_seq_no", matchesMap().extraOk().entry("total_in_bytes", greaterThanOrEqualTo(1)))
                                .extraOk()
                                .entry("_recovery_source_size", matchesMap().extraOk().entry("total_in_bytes", greaterThanOrEqualTo(0)))
                        )
                )
        );
        ...
        assertAcked(
            client().admin()
                .indices()
                .updateSettings(
                    new UpdateSettingsRequest(Settings.builder().put(IndexMetadata.SETTING_BLOCKS_WRITE, true).build(), indexName)
                )
                .actionGet()
        );
        ...
        assertMap(
            ...
            matchesMap().extraOk()
                .entry(
                    indexName,
                    matchesMap().extraOk()
                        // _id disappears here, I think this is because _id field is the only user of ES87BloomFilterPostingsFormat, which
                        // has no fields now.
                        .entry("all_fields", matchesMap().extraOk().entry("stored_fields_in_bytes", equalTo(0)))
                        .entry(
                            "fields",
                            matchesMap().extraOk()
                                .entry("_seq_no", matchesMap().extraOk().entry("total_in_bytes", equalTo(0)))
                                .extraOk()
                                .entry("_recovery_source_size", matchesMap().extraOk().entry("total_in_bytes", equalTo(0)))
                        )
                )
        );
