...
public record TimeSeriesSourceOperatorFactory(
    ...
    static class Impl extends SourceOperator {
        ...
        class TimeSeriesIterator {
            ...
            void consume() throws IOException {
                if (queue != null) {
                    ...
                    // We're lucky, only one segment, so no need to use priority queue and use segment ordinals as tsid ord.
                    for (int docId = leaf.iterator.nextDoc(); currentPagePos < maxPageSize
                        && remainingDocs > 0
                        && docId < DocIdSetIterator.NO_MORE_DOCS; docId = leaf.iterator.nextDoc()) {
                        currentPagePos++;
                        remainingDocs--;
                        // Don't append segment ord, because there is only one segment.
                        docsBuilder.appendInt(docId);

                        boolean advanced = leaf.tsids.advanceExact(docId);
                        assert advanced;
                        tsOrdBuilder.appendInt(leaf.tsids.ordValue());
                        advanced = leaf.timestamps.advanceExact(docId);
                        assert advanced;
                        if (prepared != null) {
                            timestampIntervalBuilder.appendLong(prepared.round(leaf.timestamps.nextValue()));
                        } else {
                            timestampIntervalBuilder.appendLong(leaf.timestamps.nextValue());
                        }
                    }
                }
            }
