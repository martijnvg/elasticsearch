...
class PercolatorQueryCache implements RemovalListener<PercolatorQueryCache.Key, QueryBuilder>, IndexReader.ClosedListener {
    ...
    static final Setting<ByteSizeValue> INDICES_PERCOLATOR_CACHE_SIZE_KEY =
        Setting.memorySizeSetting("indices.percolator.cache.size", "0", Setting.Property.NodeScope);
    ...
    private final MeanMetric cacheSize = new MeanMetric();

    private final Set<IndexReader.CacheKey> seenKeys = ConcurrentHashMap.newKeySet();
    private final BlockingDeque<IndexReader.CacheKey> removedKeys = new LinkedBlockingDeque<>();

    PercolatorQueryCache(Settings settings, ThreadPool threadPool) {
        ...
        this.cache = CacheBuilder.<Key, QueryBuilder>builder()
            ...
            .build();
        threadPool.scheduleWithFixedDelay(this::removeUnreferencedEntries, TimeValue.timeValueMillis(1000L), ThreadPool.Names.GENERIC);
    }

    QueryBuilder computeIfAbsent(LeafReaderContext ctx, int docId, CheckedSupplier<QueryBuilder, IOException> loader) {
        ...
        try {
            return cache.computeIfAbsent(key, key1 -> {
                if (seenKeys.add(cacheHelper.getKey())) {
                    cacheHelper.addClosedListener(PercolatorQueryCache.this);
                }
                QueryBuilder queryBuilder = loader.get();
                cacheSize.inc(estimateQueryBuilderSize(queryBuilder));
                return queryBuilder;
            });
    ...
    public void onRemoval(RemovalNotification<Key, QueryBuilder> notification) {
        cacheSize.dec(estimateQueryBuilderSize(notification.getValue()));
    }
    ...
    void removeUnreferencedEntries() {
        Set<IndexReader.CacheKey> removedKeys = new HashSet<>();
        this.removedKeys.drainTo(removedKeys);
        for (Key key : cache.keys()) {
            if (removedKeys.contains(key.readerKey)) {
